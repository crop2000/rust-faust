...
Compilation options: -a /tmp/.tmphpcpMI -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -double -ftz 0
------------------------------------------------------------ */
use faust_types::*;
...
impl Volume {
	...
	pub fn compute_arrays(&mut self, count: i32, inputs: &[&[F64] ; 2], outputs: &mut [&mut [F64] ; 2]) {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-a /tmp/.tmphpcpMI -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -double -ftz 0");
	...
	pub fn build_user_interface(&self, ui_interface: &mut dyn UI<F64>) {
		Self::build_user_interface_static(ui_interface);
	}
	
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<F64>) {
	...
	pub fn get_param(&self, param: ParamIndex) -> Option<F64> {
		match param.0 {
	...
	pub fn set_param(&mut self, param: ParamIndex, value: F64) {
		match param.0 {
	...
	pub fn compute(&mut self, count: i32, inputs: & [& [F64] ], outputs: & mut[& mut[F64] ]) {

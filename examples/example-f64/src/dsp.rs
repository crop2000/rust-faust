...
Compilation options: -a /tmp/.tmpePd7iT -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */
pub mod dsp {
    #![allow(clippy::all)]
    #![allow(unused_parens)]
...
impl FaustDsp for Volume {
	...
	fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-a /tmp/.tmpePd7iT -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
	...
	fn compute<'a>(&mut self, count: i32, inputs: & 'a [& 'a [Self::T] ; 2], outputs: & 'a mut[& 'a mut[Self::T] ; 2]) {
		...
		let [inputs0, inputs1, ..] = inputs;
			...
			let outputs1 = outputs1[..count as usize].iter_mut();let mut fSlow0: F64 = self.fConst1 * F64::powf(1e+01, 0.05 * self.fVslider0);

...
Compilation options: -a /tmp/.tmpfv00MF -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */
...
impl Volume {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-a /tmp/.tmpfv00MF -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
	...
	pub fn compute(
		...
		let mut fTemp0: F64 = self.fVbargraph0;
		let mut fSlow1: F64 = self.fConst2 * F64::powf(1e+01, 0.05 * self.fVslider1);
		let mut fTemp1: F64 = self.fVbargraph1;
		let zipped_iterators = inputs0.zip(inputs1).zip(outputs0).zip(outputs1);
		for (((input0, input1), output0), output1) in zipped_iterators {
			self.fRec1[0] = fSlow0 + self.fConst3 * self.fRec1[1];
			self.fRec0[0] = F64::max(self.fRec0[1] - self.fConst1, F64::abs(*input0 * self.fRec1[0]));
			fTemp0 = 2e+01 * F64::log10(F64::max(2.2250738585072014e-308, F64::max(0.00031622776601683794, self.fRec0[0])));
			*output0 = (self.iConst4) as F64;
			self.fRec3[0] = fSlow1 + self.fConst3 * self.fRec3[1];
			self.fRec2[0] = F64::max(self.fRec2[1] - self.fConst1, F64::abs(*input1 * self.fRec3[0]));
			fTemp1 = 2e+01 * F64::log10(F64::max(2.2250738585072014e-308, F64::max(0.00031622776601683794, self.fRec2[0])));
		...
		self.fVbargraph0 = fTemp0;
		self.fVbargraph1 = fTemp1;
...
impl FaustDsp for Volume {
    ...
    pub enum UIActiveShortname {
    channel_0_volume,
    channel_1_volume,
}
impl UIActiveShortname {
    pub fn set(&self, dsp: &mut Volume, value: FaustFloat) {
        match self {
            UIActiveShortname::channel_0_volume => dsp.fVslider0 = value,
            UIActiveShortname::channel_1_volume => dsp.fVslider1 = value,
        }
    }
}
pub enum UIPassiveShortname {
    channel_0_level,
    channel_1_level,
}
impl UIPassiveShortname {
    pub fn get(&self, dsp: &mut Volume) -> FaustFloat {
        match self {
            UIPassiveShortname::channel_0_level => dsp.fVbargraph0,
            UIPassiveShortname::channel_1_level => dsp.fVbargraph1,
        }
    }
}
    ...
    pub use dsp::Volume;
    pub use dsp::UIActiveShortname;
    pub use dsp::UIPassiveShortname;

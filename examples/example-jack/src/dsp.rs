...
Compilation options: -a /tmp/.tmpSH0bHy -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
#![allow(clippy::all)]
#![allow(unused_parens)]
...
type T = F32;
type I<'a> = &'a [&'a [T]; 2];
type O<'a> = &'a mut [&'a mut [T]; 2];
impl Volume {
    fn compute_arrays(&mut self, count: i32, inputs: &[&[T]; 2], outputs: &mut [&mut [T]; 2]) {
    ...
    fn metadata(&self, m: &mut dyn Meta) {
        ...
        m.declare("compile_options", r"-a /tmp/.tmpSH0bHy -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
    ...
    fn build_user_interface(&self, ui_interface: &mut dyn UI<T>) {
        Self::build_user_interface_static(ui_interface);
    }

    fn build_user_interface_static(ui_interface: &mut dyn UI<T>) {
    ...
    fn get_param(&self, param: ParamIndex) -> Option<T> {
        match param.0 {
    ...
    fn set_param(&mut self, param: ParamIndex, value: T) {
        match param.0 {
...
// this should be from the architecture file

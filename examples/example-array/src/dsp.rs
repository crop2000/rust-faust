...
Compilation options: -a /tmp/.tmpc7S5rE -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
pub mod dsp {
    #![allow(clippy::all)]
    #![allow(unused_parens)]
...
impl FaustDsp for Volume {
	...
	fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-a /tmp/.tmpc7S5rE -lang rust -ct 1 -cn Volume -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	fn compute<'a>(&mut self, count: i32, inputs: & 'a [& 'a [Self::T] ; 2], outputs: & 'a mut[& 'a mut[Self::T] ; 2]) {
		...
		let [inputs0, inputs1, ..] = inputs;
			...
			let inputs1 = inputs1[..count as usize].iter();let [outputs0, outputs1, ..] = outputs;
			let outputs0 = outputs0[..count as usize].iter_mut();
			let outputs1 = outputs1[..count as usize].iter_mut();let mut fSlow0: F32 = self.fConst1 * F32::powf(1e+01, 0.05 * self.fVslider0);
		let zipped_iterators = inputs0.zip(inputs1).zip(outputs0).zip(outputs1);
		for (((input0, input1), output0), output1) in zipped_iterators {
			self.fRec0[0] = fSlow0 + self.fConst2 * self.fRec0[1];
			let mut fTemp0: F32 = *input0;
			let mut fTemp1: F32 = *input1;
			self.fRec1[0] = F32::max(self.fRec1[1] - self.fConst3, F32::abs(0.5 * self.fRec0[0] * (fTemp0 + fTemp1)));
			self.fVbargraph0 = 2e+01 * F32::log10(F32::max(1.1754944e-38, F32::max(0.00031622776, self.fRec1[0])));

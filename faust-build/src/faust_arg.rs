...
pub trait FaustArgs2Args {
    fn to_args(&self) -> Vec<&OsStr>;
}

impl<'a> FaustArgs2Args for FaustArg<'a> {
    fn to_args(&self) -> Vec<&OsStr> {
        match self {
            FaustArg::Arg(arg) => vec![arg.as_ref()],
            FaustArg::ArchFile(path_buf) => vec!["-a".as_ref() as &OsStr, path_buf.as_ref()],
            FaustArg::ImportDir(path_buf) => vec!["-I".as_ref() as &OsStr, path_buf.as_ref()],
            FaustArg::Output(path_buf) => vec!["-o".as_ref() as &OsStr, path_buf.as_ref()],
            FaustArg::Xml() => vec!["-xml".as_ref() as &OsStr],
            FaustArg::Json() => vec!["-json".as_ref() as &OsStr],
            FaustArg::ExtraOutputDir(path_buf) => vec!["-O".as_ref() as &OsStr, path_buf.as_ref()],
            FaustArg::Lang(name) => vec!["-lang".as_ref() as &OsStr, name.as_ref()],
            FaustArg::Single => vec!["-single".as_ref() as &OsStr],
            FaustArg::Double => vec!["-double".as_ref() as &OsStr],
            FaustArg::OneSample => vec!["-os".as_ref() as &OsStr],
            FaustArg::ExternalControl => vec!["-ec".as_ref() as &OsStr],
            FaustArg::ComputeMix => vec!["-cm".as_ref() as &OsStr],
            FaustArg::StructName(name) => vec!["-cn".as_ref() as &OsStr, name.as_ref()],
            FaustArg::ProcessName(name) => vec!["-pn".as_ref() as &OsStr, name.as_ref()],
            FaustArg::InPlace => vec!["-inpl".as_ref() as &OsStr],
            FaustArg::NoReprC => vec!["-noreprc".as_ref() as &OsStr],
            FaustArg::Timeout(t) => {
                vec!["-t".as_ref() as &OsStr, t.as_ref()]
            }
            FaustArg::DspFile(path_buf) => vec![path_buf.as_ref()],
        }
    }
}

impl<'a> FaustArgs2Args for Vec<FaustArg<'a>> {
    fn to_args(&self) -> Vec<&OsStr> {
        self.iter().flat_map(|fa| fa.to_args()).collect()
    }
}

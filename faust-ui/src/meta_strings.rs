...
fn lib_map_to_tokenstream(lib_map: &HashMap<&str, Vec<&Meta>>) -> Vec<TokenStream> {
    let libs = lib_map
        .iter()
        .map(|(lib, v)| {
...
fn get_lib_item_map(faust_json: &faust_json::FaustJson) -> HashMap<&str, Vec<&Meta>> {
    let mut lib_map = HashMap::<&str, Vec<&Meta>>::new();
